...
describe('mapCreatorErrorToCode', () => {
  it('maps validation errors', () => {
    expect(mapCreatorErrorToCode({ missingRequired: true }, 'validation')).toBe(
      'creator_validation_missing_required',
    );
    expect(mapCreatorErrorToCode({ invalidLength: true }, 'validation')).toBe(
      'creator_validation_invalid_length',
    );
    expect(mapCreatorErrorToCode({ invalidPrice: true }, 'validation')).toBe(
      'creator_validation_invalid_price',
    );
    expect(mapCreatorErrorToCode({ missingMedia: true }, 'validation')).toBe(
      'creator_validation_missing_media',
    );
    expect(mapCreatorErrorToCode({ missingCollection: true }, 'validation')).toBe(
  ...
  it('maps request and response', () => {
    ...
    expect(mapCreatorErrorToCode({ serverUnavailable: true }, 'response')).toBe(
