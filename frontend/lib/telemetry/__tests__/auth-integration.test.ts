...
describe('authInstrumentation integration', () => {
  it('emits login submit, success, and failure events with correct payloads', () => {
    const trackSpy = jest
      .spyOn(require('../index').telemetry, 'track')
      .mockImplementation(() => {});
    ...
    expect(trackSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ auth_method: 'wallet', surface: 'login_page', attempt_id }),
    );
    expect(trackSpy).toHaveBeenCalledWith(
      'auth_login_succeeded',
      expect.objectContaining({ auth_method: 'wallet', attempt_id, had_wallet_connected: true }),
    );
    expect(trackSpy).toHaveBeenCalledWith(
      'auth_login_failed',
  ...
  it('emits register submit, success, and failure events with correct payloads', () => {
    ...
    expect(trackSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ auth_method: 'email', attempt_id, redirects_to_login: true }),
    );
    expect(trackSpy).toHaveBeenCalledWith(
