...
export const authInstrumentation = {
  ...
  loginSuccess({
    ...
    emitAuthEvent(EVENT_NAMES.authLoginSucceeded, {
      auth_method,
      latency_ms,
      attempt_id,
      had_wallet_connected,
  ...
  loginFailed({
    ...
    emitAuthEvent(EVENT_NAMES.authLoginFailed, {
      auth_method,
      attempt_id,
      latency_ms,
      error_code,
  ...
  submitRegister({
    ...
    const attempt_id = generateAttemptId();
    emitAuthEvent(EVENT_NAMES.authRegisterSubmitted, {
      auth_method,
      surface,
      attempt_id,
      has_optional_username,
  ...
  registerSuccess({
    ...
    const latency_ms = Math.round(nowMs() - startMs);
    emitAuthEvent(EVENT_NAMES.authRegisterSucceeded, {
      auth_method,
      attempt_id,
      latency_ms,
      redirects_to_login,
  ...
  registerFailed({
    ...
    const latency_ms = Math.round(nowMs() - startMs);
    const error_code = mapAuthErrorToCode(error);
    emitAuthEvent(EVENT_NAMES.authRegisterFailed, {
      auth_method,
      attempt_id,
      latency_ms,
      error_code,
