...
export async function normalizeApiError(error: unknown): Promise<AppApiError> {
  ...
  if (error instanceof TypeError && error.message.toLowerCase().includes('failed to fetch')) {
    return new AppApiError({
      code: 'NETWORK_ERROR',
      message: 'Network disconnected or server unreachable. Please check your internet connection.',
...
export async function parseResponseError(res: Response): Promise<AppApiError> {
  ...
  const msg = payload?.message || payload?.error || `HTTP error! status: ${status}`;
 ...
 */
export function getValidationFieldMessage(error: unknown, fieldName: string): string | undefined {
  if (!error || typeof error !== 'object') return undefined;

  const apiError = error as any;

  // 1. Check if the error object directly contains a specific field validation payload
  if (apiError.validationErrors && typeof apiError.validationErrors === 'object') {
