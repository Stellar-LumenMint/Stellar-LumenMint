...
export function validateTranslations(): TranslationValidationResult {
  ...
  for (const key in enFlat) {
    if (enFlat[key] === '' || enFlat[key] === null || enFlat[key] === undefined) {
      result.emptyValues.push(`en.${key}`);
    }
  }

  for (const key in frFlat) {
    if (frFlat[key] === '' || frFlat[key] === null || frFlat[key] === undefined) {
...
export function logValidationResults(): void {
  ...
  if (result.isValid) {
    ...
    if (result.inconsistentTypes.length > 0) {
      ...
      result.inconsistentTypes.forEach((inconsistency) => console.log(`  - ${inconsistency}`));
    }
  ...
  console.log('\n📊 Summary:');
  console.log(`  - Total keys in English: ${Object.keys(flattenObject(enCommon)).length}`);
  console.log(`  - Total keys in French: ${Object.keys(flattenObject(frCommon)).length}`);
