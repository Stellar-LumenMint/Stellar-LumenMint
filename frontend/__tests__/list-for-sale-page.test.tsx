...
const mockedHook = useCreatorListings as jest.MockedFunction<typeof useCreatorListings>;
...
describe('ListNFTsForSale page', () => {
  ...
  it('creates a listing through the inline form', async () => {
    ...
    await waitFor(() =>
      expect(createListing).toHaveBeenCalledWith(expect.objectContaining({ nftKey: 'C1:t1' }), {
        price: 25,
        currency: 'XLM',
      }),
    );
  ...
  it('cancels an active listing', async () => {
    ...
    fireEvent.click(within(grid).getByRole('button', { name: /cancel listing/i }));
