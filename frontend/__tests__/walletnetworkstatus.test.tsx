...
describe('WalletNetworkStatus', () => {
  ...
  it("warns when the wallet network differs from the app's expected network", () => {
    render(<WalletNetworkStatus network="mainnet" expectedNetwork="testnet" />);
    const badge = screen.getByRole('status');
    expect(badge).toHaveTextContent('wrong network');
    expect(badge).toHaveAccessibleName('Wallet connected to mainnet, app expects testnet');
  });

  it('does not warn when the networks match', () => {
    render(<WalletNetworkStatus network="testnet" expectedNetwork="testnet" />);
    const badge = screen.getByRole('status');
