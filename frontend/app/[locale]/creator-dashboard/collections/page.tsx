...
export default function CollectionsPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-lumen-text mb-4">Collections</h1>
          <p className="text-lumen-subtext text-lg">No collections yet. Create your first one!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="py-8 px-4">
      <h1 className="text-3xl font-bold text-lumen-text mb-8">Collections</h1>
      ...
      <WidgetErrorBoundary widgetName="collections grid" onRetry={handleRetry}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {collections.map((col) => (
            <div
              key={col.id}
              className="rounded-xl border border-lumen-border bg-lumen-card overflow-hidden hover:border-lumen-teal/40 transition-colors"
            >
              {/* Banner / Image */}
              <div className="relative h-32 bg-gradient-to-br from-[#0D1117] to-[#1C2433]">
              ...
              <div className="p-4 pt-2">
                <h2 className="text-lumen-text font-semibold truncate">{col.name}</h2>
                <p className="text-lumen-subtext text-xs mb-1">{col.symbol}</p>
                {col.description && (
                  <p className="text-lumen-subtext text-sm line-clamp-2">{col.description}</p>
                )}
              </div>
            </div>
          ))}
