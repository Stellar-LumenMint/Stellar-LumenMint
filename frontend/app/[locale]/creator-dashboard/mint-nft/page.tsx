  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!contractAddress.trim()) {
      setFieldErrors((prev) => ({
        ...prev,
        contractAddress:
          t('mintNFT.errors.contractAddressRequired') ?? 'Contract address is required.',
      }));
      return;
    }
  ...
  return (
    <div className="min-h-[100svh] mt-32 flex items-center justify-center px-4 py-6">
      <form
        ...
        className="bg-lumen-card p-8 rounded-2xl shadow-lg w-full max-w-md border border-lumen-border space-y-4"
      >
        <h1 className="text-3xl font-bold text-center text-lumen-text mb-2">
          {t('mintNFT.title')}
        </h1>
        ...
        {collections.length > 0 ? (
          <div>
            <label className="block text-sm text-lumen-subtext mb-1">
            ...
            <select
              ...
              className="w-full px-4 py-2 rounded bg-lumen-background text-lumen-text border border-lumen-border focus:outline-none focus:ring-1 focus:ring-lumen-primary text-sm"
              required
            >
            ...
            {fieldErrors.collection && (
              <p className="text-xs text-red-400 mt-1 font-medium">{fieldErrors.collection}</p>
            )}
          </div>
        ) : (
          <div className="p-3 bg-amber-950/40 text-amber-300 border border-amber-500/20 rounded-lg text-xs">
            No smart asset collections found. Please construct a parent collection pipeline before
            minting digital properties.
          </div>
        )}

        <div>
          <label className="block text-sm text-lumen-subtext mb-1">{t('mintNFT.title')} *</label>
          <input
            type="text"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            className={`w-full px-4 py-2 rounded bg-lumen-background text-lumen-text border text-sm focus:outline-none focus:ring-1 ${
              fieldErrors.title
                ? 'border-red-500 focus:ring-red-500'
                : 'border-lumen-border focus:ring-lumen-primary'
            }`}
            required
          />
          {fieldErrors.title && (
            <p className="text-xs text-red-400 mt-1 font-medium">{fieldErrors.title}</p>
          )}
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className={`w-full px-4 py-2 rounded bg-lumen-bg text-lumen-text border text-sm focus:outline-none focus:ring-1 ${
              fieldErrors.description
                ? 'border-red-500 focus:ring-red-500'
                : 'border-lumen-border focus:ring-lumen-teal'
            }`}
            required
            rows={3}
          />
          {fieldErrors.description && (
            <p className="text-xs text-red-400 mt-1 font-medium">{fieldErrors.description}</p>
          )}
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-lumen-subtext mb-1">{t('mintNFT.price')} *</label>
            <input
              type="number"
              step="0.00001"
              min="0"
            ...
            />
            {fieldErrors.price && (
              <p className="text-xs text-red-400 mt-1 font-medium">{fieldErrors.price}</p>
            )}
          </div>
          <div>
            <label className="block text-sm text-lumen-subtext mb-1">{t('mintNFT.currency')}</label>
            <select
              value={currency}
              onChange={(e) => setCurrency(e.target.value)}
              className="w-full px-4 py-2 rounded bg-lumen-bg text-lumen-text border border-lumen-border text-sm focus:outline-none focus:ring-1 focus:ring-lumen-teal"
            >
        ...
        <div>
          <label className="block text-sm text-lumen-subtext mb-1">Contract Address *</label>
          <input
            type="text"
          ...
          />
          {fieldErrors.contractAddress && (
            <p className="text-xs text-red-400 mt-1 font-medium">{fieldErrors.contractAddress}</p>
          )}
        </div>

        <div>
          <label className="block text-sm text-lumen-subtext mb-2">{t('mintNFT.nftImage')} *</label>
          <FileDropZone
            onFilesSelected={setFiles}
            accept={['image/*']}
            maxSizeMB={10}
            className="border border-lumen-border rounded-lg bg-lumen-bg hover:border-lumen-teal/60 transition-colors"
            dropZoneText={t('mintNFT.dragDropText')}
            dropZoneTextClass="text-lumen-subtext"
          />
          {fieldErrors.files && (
            <p className="text-xs text-red-400 mt-1 font-medium">{fieldErrors.files}</p>
          )}
          {files.length > 0 && (
            <p className="mt-2 text-xs text-lumen-teal truncate">
              Ready: {files[0].file.name} ({(files[0].file.size / 1024 / 1024).toFixed(2)} MB)
            </p>
          )}
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-lumen-teal text-[#0D1117] py-2.5 px-4 rounded-lg hover:bg-lumen-hover transition font-medium disabled:opacity-40 text-sm"
        >
