    ...
    Promise.allSettled([
      ...
      .then(([collectionsRes, nftsRes, earningsRes, txsRes]) => {
        ...
        if (txsRes.status === 'fulfilled') {
          ...
          totalTransactions = txArray.filter(
            (tx: any) =>
              tx.sellerId === user.id && (tx.state === 'completed' || tx.state === 'COMPLETED'),
          ).length;
        }
          ...
          txArray.forEach((tx: any) => {
            if (tx.state === 'completed' || tx.state === 'COMPLETED') {
              ...
              recentActivities.push({
                id: `tx-${tx.id}`,
                action: isSale
                  ? `NFT #${tx.nftTokenId || 'Unknown'} sold`
                  : `NFT #${tx.nftTokenId || 'Unknown'} purchased`,
      ...
      .catch((err) => {
        ...
        setErrors({
          ...
          activities: 'Failed to load recent activity feed.',
        });
        setLoading({ stats: false, activities: false });
      });
  }, [user?.id]);
