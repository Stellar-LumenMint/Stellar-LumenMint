...
function CountdownTimer({ endTime, t }: { endTime: string; t: (key: string) => string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <Clock
        className={cn('h-5 w-5', isEndingSoon ? 'text-red-400 animate-pulse' : 'text-purple-400')}
      />
      <div className="flex items-center gap-3">
        {timeLeft.d > 0 && (
          <TimeUnit
            value={timeLeft.d}
            label={t('auctionDetail.countdown.days')}
            highlight={isEndingSoon}
          />
        )}
        <TimeUnit
          value={timeLeft.h}
          label={t('auctionDetail.countdown.hours')}
          highlight={isEndingSoon}
        />
        <span className="text-lg font-light text-gray-600">:</span>
        <TimeUnit
          value={timeLeft.m}
          label={t('auctionDetail.countdown.minutes')}
          highlight={isEndingSoon}
        />
        <span className="text-lg font-light text-gray-600">:</span>
        <TimeUnit
          value={timeLeft.s}
          label={t('auctionDetail.countdown.seconds')}
          highlight={isEndingSoon}
        />
      </div>
    </div>
...
export default function AuctionDetailPage() {
  ...
  const timeLeft = useMemo(() => (auction ? getTimeLeft(auction.endTime) : null), [auction]);

  const isEnded = timeLeft?.ended ?? false;
  const minBid = auction ? parseFloat(auction.highestBid?.amount || auction.startPrice) + 0.01 : 0;
  ...
  const sortedBids = auction.bids
    ? [...auction.bids].sort((a, b) => parseFloat(b.amount) - parseFloat(a.amount))
    : [];
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    title={
                      isWatched
                        ? t('auctionDetail.removeFromWatchlist')
                        : t('auctionDetail.addToWatchlist')
                    }
                  >
