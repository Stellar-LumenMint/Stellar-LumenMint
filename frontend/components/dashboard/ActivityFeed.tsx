...
export function ActivityFeed({ loading, error, recentActivities }: ActivityFeedProps) {
      ...
      {loading ? (
        <div className="space-y-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="flex items-center justify-between p-4 rounded-lg bg-muted/40 animate-pulse"
            >
        ...
        <div className="text-center py-12 bg-muted/10 border border-dashed border-border rounded-xl">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">
            Start minting NFTs or listing them for sale to populate this feed.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {recentActivities.map((activity, index) => {
            ...
            try {
              const dateObj =
                activity.time instanceof Date ? activity.time : new Date(activity.time);
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-3.5 h-3.5 rounded-full border border-background/20 ${
                      activity.type === 'sale'
                  ...
                  />
                  <div>
                    <div className="text-card-foreground font-semibold text-sm md:text-base">
                      {activity.action}
                    </div>
                    {displayTime && (
                      <div className="text-xs text-muted-foreground mt-0.5">{displayTime}</div>
                    )}
                  </div>
                </div>
                <div className="text-card-foreground font-bold font-mono text-sm md:text-base bg-muted/30 px-3 py-1 rounded-lg border border-border/20">
                  {activity.amount}
                </div>
              </div>
            );
