...
describe('AuctionCountdown', () => {
  ...
  it('renders remaining time using the server clock offset', () => {
    ...
    render(
      <AuctionCountdown endTime={endTime} serverTimeOnMount={new Date(serverNow).toISOString()} />,
    );
    expect(screen.getByTestId('auction-countdown')).toHaveTextContent('01:30:00');
  });
  ...
  it('falls back to the client clock when server time is missing', () => {
    ...
    expect(screen.getByTestId('auction-countdown')).toHaveTextContent('45:00');
  });

  it('falls back to the client clock when server time is invalid', () => {
    const endTime = new Date(Date.now() + 10 * 60 * 1000).toISOString();
    render(<AuctionCountdown endTime={endTime} serverTimeOnMount="not-a-date" />);
    expect(screen.getByTestId('auction-countdown')).toHaveTextContent('10:00');
  });
  ...
  it('shows Auction Ended at zero and fires onAuctionEnd', () => {
    ...
    render(<AuctionCountdown endTime={endTime} onAuctionEnd={onAuctionEnd} />);
    expect(screen.getByTestId('auction-countdown')).toHaveTextContent('Auction Ended');
  });
