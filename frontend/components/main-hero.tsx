...
export function MainHero() {
  ...
  const heroCTAAssignment = useExperimentVariant('hero-cta-placement-2026-q2');
  ...
  const handleRegisterClick = () => {
    if (!heroCTAAssignment || !exposureSessionIdRef.current || !exposureTimestampRef.current)
      return;
    telemetry.track(
  ...
  return (
    <div className="relative flex flex-col lg:flex-row gap-8 lg:gap-12 items-center py-8 sm:py-12 md:py-16 lg:py-20 px-4 sm:px-8 lg:px-0 mt-8 sm:mt-12 md:mt-16 overflow-hidden">
      {/* Ambient glow orbs */}
      <div
        className="absolute -top-32 -left-32 w-96 h-96 bg-[#00D4FF]/5 rounded-full blur-[120px] animate-lm-glow-pulse"
        aria-hidden="true"
      />
      <div
        className="absolute -bottom-32 -right-32 w-96 h-96 bg-[#7B6FFF]/5 rounded-full blur-[120px] animate-lm-glow-pulse"
        aria-hidden="true"
        style={{ animationDelay: '1.5s' }}
      />
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-r from-[#00D4FF]/3 to-[#7B6FFF]/3 rounded-full blur-[150px]"
        aria-hidden="true"
      />
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 pt-2 sm:pt-4 lm-stagger-4">
          <Link href={`/${locale}/auth/register`}>
            ...
            >
              ...
              <span
                className="absolute inset-0 bg-gradient-to-r from-[#00D4FF] to-[#7B6FFF] opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                aria-hidden="true"
              />
            </Button>
          </Link>
          <Link href={`/${locale}/marketplace`}>
            ...
            >
              <span
                className="absolute inset-0 rounded-full bg-gradient-to-r from-[#00D4FF]/10 to-[#7B6FFF]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                aria-hidden="true"
              />
              <span className="relative z-10 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 pt-6 sm:pt-8 max-w-lg lm-stagger-5">
          {features.map(({ icon: Icon, value, label, desc }, idx) => (
            ...
            >
              {/* Hover glow */}
              <div
                className="absolute -inset-px bg-gradient-to-br from-[#00D4FF]/10 to-[#7B6FFF]/10 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                aria-hidden="true"
              />
              ...
              <div className="text-xs sm:text-sm text-[#8A9BB0] font-medium">{label}</div>
              <p className="text-[10px] text-[#6B7A8D] mt-0.5">{desc}</p>

              {/* Top accent */}
              <div
                className="absolute top-0 left-1/4 right-1/4 h-[1px] bg-gradient-to-r from-transparent via-[#00D4FF]/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                aria-hidden="true"
              />
            </div>
          ))}
      ...
      <div className="w-full md:w-1/2 lg:flex hidden justify-center md:justify-end items-center mt-8 md:mt-0">
        <div className="relative max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl w-full md:w-[90%] lg:w-[85%] -mt-8 sm:-mt-12 md:-mt-16 lg:-mt-24 md:mr-4 lg:mr-8">
          <div className="relative h-[260px] sm:h-[340px] md:h-[400px] lg:h-[500px] flex items-center justify-center">
            {/* Decorative rings */}
            <div
              className="absolute w-[90%] h-[90%] border border-[#00D4FF]/10 rounded-full animate-lm-spin-slow"
              aria-hidden="true"
            />
            <div
              className="absolute w-[70%] h-[70%] border border-[#7B6FFF]/10 rounded-full animate-lm-spin-slow"
              aria-hidden="true"
