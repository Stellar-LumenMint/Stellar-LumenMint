...
//! that never extends its TTLs will eventually stop working — an especially
//! sharp failure mode for an NFT contract, where the collection metadata,
//! balances and per-token ownership that users paid for must outlive the
//! deployment.
//!
//! Two rules matter:
//!
//! 1. Contract **instance** entries (admin, supply counters, role grants, the
//!    contract code itself) share one TTL. A single
//!    `instance().extend_ttl(...)` covers all of them, so it is enough to bump
//!    the instance on every entry point that mutates state.
...
//! days and top them up to 60 days, which gives operators a wide window to
//! call the contract and refresh them.
